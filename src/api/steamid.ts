  ...
  switch (type) {
    ...
    case SteamIDType.Invalid:
      if (input.match(steamFixedURLRegex)) {
        ...
        return (
          splitInput[splitInput.length - 1] || splitInput[splitInput.length - 2]
        );
      }
  }
  return undefined;
...
export function convertSteamID64toSteamID3(steamID64: string) {
  ...
  const output =
    '[' +
    steamID3LetterTable.get(type) +
    ':1:' +
    (accountNumber * 2n + y).toString() +
    ']';
  return output;
}
