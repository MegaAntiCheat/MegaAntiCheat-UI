...
const PageSelector: React.FC<PageSelectorProps> = ({
  ...
  const getButtonsToDisplay = (page: number): number[] => {
    ...
    if (page <= middleValue) {
      Array.from({ length: page }, (_, index) => index + 1).forEach((n) =>
        pages.add(n),
      );
  ...
  return (
    <div className="flex justify-center">
      <button
        ...
        className={`min-w-pageselectorbutton p-2 ${
          selectedPage === 1 ? 'text-black' : ''
        }`}
      >
        {'<'}
      </button>
        ...
        (page) =>
          buttonsToDisplay.includes(page) && (
            <button
              ...
              className={`min-w-pageselectorbutton p-2 ${
                page === selectedPage ? 'bg-white text-black' : ''
              }`}
            >
              {page + ' '}
            </button>
      ...
      <button
        ...
        className={`min-w-pageselectorbutton p-2 ${
          selectedPage === totalPages ? 'text-black' : ''
        }`}
      >
        {'>'}
      </button>
